...
- A Cloudflare tunnel runs on Docker and routes `web-int.pub-o.com` to the `pubo_web` service. Treat `https://web-int.pub-o.com` as the public web URL for Pub-O; the Docker-side origin is `pubo_web:8080`. If a separate API hostname is needed, use `api-int.pub-o.com`.
