...
- A Cloudflare tunnel runs on Docker. Treat `https://int.app.pub-o.com` as the public app URL, `https://int.dash.pub-o.com` as the public admin dashboard URL, and `https://int.api.pub-o.com` as the public API URL. Docker-side origins are `pubo_app:8080` for the app, `pubo_dash:8080` for the admin dashboard, and `pubo_api:4000` for API.
